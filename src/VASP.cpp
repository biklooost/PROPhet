...
Structure VASP::read_structure(string filename)
{
  ...
  if (vectors.size() != new_struct.Natom) {
    ERROR("Atoms were not read correctly");
  }
...
REAL VASP::get_property(string property, string directory)
{
  ...
  if (property == "energy") {
    ...
    if (nodes.empty()) {
      ERROR("Could not get energy from "+directory+"/vasprun.xml");
    }
    ...
    for (int i=0; i<Nkpoints; i++) {
      ...
      if (Ek.size() < VB+1) {
        ERROR("Problem reading k-points from "+filename);
      }
...
Grid_data VASP::get_density(string directory, int step)
{
  ...
  if (Ntotal <= 0) {
    ERROR("Did not read density correctly from file \""+directory+"/CHGCAR"+"\"");
  }
