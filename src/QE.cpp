...
Structure QE::read_structure(string prefix)
{
  ...
  while (tvector >> tnumber) {
    temps.a.push_back(tnumber*0.529177249);
  }
  tvector.str("");
  tvector.clear();
  ...
  tvector << node.back().child_value();
  while (tvector >> tnumber) {
    temps.b.push_back(tnumber*0.529177249);
  }
  tvector.str("");
  tvector.clear();
  ...
  tvector << node.back().child_value();
  while (tvector >> tnumber) {
    temps.c.push_back(tnumber*0.529177249);
  }
  tvector.str("");
  tvector.clear();
...
REAL QE::read_band_gap(string prefix)
{
  ...
  if(nodes.empty()) {
    cout << "Unable to read K-Points from " << datafile << "\n";
  }
  nodes = xml.get_all_nodes_by_name("NUMBER_OF_BANDS");
  if(nodes.empty()) {
    cout << "Unable to read Number of bands from " << datafile << "\n";
  }
...
Grid_data QE::get_density(string prefix, int step)
{
  ...
  if (file.is_open()) {
    ...
    cout << "Unable to Open Charge Density";
  }
}
