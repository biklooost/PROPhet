...
//      cout << "############" << endl;
  ...
  for (int i=0; i<atom_types.size(); i++) {
    ...
    if (!F_in.input_file().empty()) {
      ...
      if (!file.is_open()) {
        ERROR("Found atom type "+atom.atomic_symbol()+" but could not open restart file '"+filename+"'");
      }
      if (mpi->io_node()) {
    ...
    if (!F_in.input_file().empty()) {
      ...
      file.open(filename.c_str());
      if (!file.is_open()) {
        ERROR("Could not open network restart file '"+filename+"'");
      }
...
void Potential::forces()
{
  ...
  for (int i_sys=0; i_sys < systems.size(); i_sys++) {
    ...
    for (int i=0; i < systems[i_sys]->structure.pos.size(); i++) {
      f[i] = new double[3];
    }
...
void Potential::insert_atom_type(int atom_number, char* filename, System* system)
{
  ...
  file.open(filename);
  if (!file.is_open()) {
    ERROR("Could not open parameter file \""+string(filename)+"\"");
  }
