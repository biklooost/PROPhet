...
class Parallel {
...
#ifdef USE_MPI  
  int io_node();
  inline int rank() { return my_rank; }
  inline int Nprocs() { return my_Nprocs; }

  inline void Abort() { MPI_Barrier(MPI_COMM_WORLD);MPI_Finalize(); exit(1); }
 ...
 private:
  ...
  inline MPI_Datatype MPI_Type(int in) { return MPI_INT;}
  inline MPI_Datatype MPI_Type(double in) { return MPI_DOUBLE;}
  inline MPI_Datatype MPI_Type(float in) { return MPI_FLOAT; }
  inline MPI_Datatype MPI_Type(long unsigned int in) { return MPI_UNSIGNED_LONG; }
  inline MPI_Datatype MPI_Type(long double in) { return MPI_LONG_DOUBLE; }
