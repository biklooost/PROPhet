...
public:
  ...
  inline virtual void set_means(vector<REAL> means)
  {
    params.input_mean = means;
  }
  inline virtual void set_variances(vector<REAL> variances)
  {
    params.input_variance = variances;
  }
  inline virtual bool is_preconditioned()
  {
    return params.input_mean.size();
  }
  ...
  virtual inline const vector<REAL>& get_gradient()
  {
    return my_dOutput_dParameters;
  }
  virtual void init_optimizer(Optimizer* opt);

  inline void set_output_mean(REAL new_mean)
  {
    output_mean = new_mean;
  }
  inline void set_output_variance(REAL new_variance)
  {
