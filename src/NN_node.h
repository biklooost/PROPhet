...
public:
  ...
  inline REAL evaluate(const vector<REAL> &in) {
    if (Ninputs != in.size()) { ERROR("Wrong number of inputs passed to node"); }
  ...
  inline vector<REAL> dOutput_dParameters() { return my_dOutput_dParameters; }
  inline REAL dOutput_dParameters(int i) { return my_dOutput_dParameters[i]; }
  
  inline vector<REAL> dOutput_dInputs() { return my_dOutput_dInputs; }
  inline REAL dOutput_dInputs(int i) { return my_dOutput_dInputs[i]; }
  
  inline int set_index(int new_index) { my_index = new_index; }
