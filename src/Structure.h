...
class Structure {
	...
	inline bool is_initialized() { return my_is_initialized; }
	...
	inline void To_Cart(vector <REAL>& proj ) {
	  ...
	  if (recalc_exp1) { G_exp1 = exp_term(-row[2]*(R*R)); } 
	  if (recalc_exp2) { G_exp2 = exp_term(-row[2]*(Ru*Ru)); }
	  if (recalc_fc1) { G_fc1 = fc(R/row[0],row[1]); }
	  if (recalc_fc2) { G_fc2 = fc(Ru/row[0],row[1]); }
