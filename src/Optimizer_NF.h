...
template <typename Type> class Optimizer {
 ...
 public:
  ...
  inline void set_debug(REAL new_debug) { debug = new_debug; }
  
  inline REAL alpha() { return my_alpha; }
 ...
 private:
  ...
  inline REAL dot_product(vector<REAL> v1, vector<REAL> v2) {
    int N=v1.size();
    if (N != v2.size()) { ERROR("Cannot take dot product of unequal length vectors"); }
    REAL result = 0.0;
    for (int i=0; i<N; i++) {
  ...
  template <typename type> inline int sign(type value) { return (type(0) < value)-(value < type(0)); }
