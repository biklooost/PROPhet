...
class Functional_params {
 ...
 public:
  ...
  inline int is_key(string key) { return my_keys.count(key); }
  
  
  inline int hidden(int i) { return my_hidden.at(i); }
  ...
  inline string inputs(int i) { return my_inputs.at(i); }
  inline vector<string> inputs() { return my_inputs; }
  ...
  inline int Ninputs() { return my_inputs.size(); }
  ...
  inline string output() { return my_output; }
  inline void set_output(string new_out) { my_output = new_out; }
  
  inline int Nlayers() { return my_hidden.size(); }
  inline int NNodes(int i) { if (i<0) { return 0; } return my_hidden.at(i); }

  inline REAL threshold() { return my_threshold; }
  
  inline string transfer_function() { return my_transfer_function; }
  ...
  inline string Network_type() { return my_network_type; }
  
  void print(ostream &out);
  void read(istream &in);
  
  inline int Niterations() { return my_Niterations; }
  inline int Ncheckpoint() { return my_Ncheckpoint; }
  inline int Nprint() { return my_Nprint; }
  inline int Ncycles() { return my_Ncycles; }
  inline int lambda_max() { return my_lambda_max; }

  inline REAL line_min_epsilon() { return my_line_min_epsilon; }
  inline REAL sd_momentum() { return my_sd_momentum; }
  
  inline string training_algorithm() { return my_training_algorithm; }
  
  inline REAL debug() { return my_debug; }

  vector<REAL> input_mean;
  vector<REAL> input_variance;


  inline bool output_precondition() { return my_output_precondition; }
  REAL output_mean;
  REAL output_variance;
  
  inline int sample_step() { return my_downsample; }
  ...
  inline REAL regularization() { return my_regularization; }
  
  inline vector<int> atomic_numbers() { return my_atomic_numbers; }
  inline vector<string> atomic_symbols() { return my_atomic_symbols; }
