...
void Structure::Allocate_G(int Gtype, vector <REAL>& row) {
  ...
  if(Gtype ==1){
    if(row.size() == 2) { 
      G1p.push_back(row);
    }else{ERROR("Improper number of parameters");}
  }else if(Gtype ==2) {
    if(row.size() == 4) {
      G2p.push_back(row);
    }else{ERROR("Improper number of parameters");}
	...
	this->nc.clear();
	  ...
	  this->nb.push_back(this->b[i]/normb);
        ...
        for (unsigned i=0; i< this->pos.size(); i++) {
            ...
            this->To_Frac(proj);
            for(int j = 0; j<3; j++) {
                if (proj[j] < 0) {proj[j]+=1;} else if (proj[j] >1) {proj[j]-=1;}
                this->pos[i][j] = proj[j];
            }
        }
      }
...
//                       SET_NEIGHBOR
// ########################################################
// Determines the neighbor list for the requird cutoff 
// distance.
